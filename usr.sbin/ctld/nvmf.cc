...
struct nvmf_io_portal final : public nvmf_portal {
	nvmf_io_portal(struct portal_group *pg, const char *listen,
	    enum portal_protocol protocol, freebsd::addrinfo_up ai,
...
struct nvmf_transport_group final : public portal_group {
	...
	bool add_portal(const char *value, enum portal_protocol protocol)
	    override;
...
private:
	struct nvmf_association_params init_aparams(
	    enum portal_protocol protocol);
...
nvmf_transport_group::init_aparams(enum portal_protocol protocol)
{
...
bool
nvmf_transport_group::add_portal(const char *value,
    enum portal_protocol protocol)
{
	...
	portal_up portal;
	if (protocol == NVME_DISCOVERY_TCP) {
...
{
	add_portal("0.0.0.0", NVME_DISCOVERY_TCP);
	add_portal("[::]", NVME_DISCOVERY_TCP);
	add_portal("0.0.0.0", NVME_TCP);
	add_portal("[::]", NVME_TCP);
}
